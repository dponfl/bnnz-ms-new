...
const moduleName = 'parsers:inst:rapid-api-petrichor:get-likes-joi';


module.exports = {


  friendlyName: 'parsers:inst:rapid-api-petrichor:get-likes-joi',
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      const parserAction = sails.config.custom.config.parsers.inst.rapidApiPetrichor.paths.getLikes;
      ...
      if (
        _.isNil(requestRes.data.shortcode_media.shortcode)
        || requestRes.data.shortcode_media.shortcode !== input.shortCode
      ) {
        ...
        await LogProcessor.error({
          message: sails.config.custom.INST_PARSER_WRONG_RESPONSE_DATA.message + ': Invalid response shortcode',
          clientGuid,
          accountGuid,
          // requestId: null,
          // childRequestId: null,
        ...
        return exits.success({
          ...
          message: `${moduleName} performed with error`,
          payload: {
            error: 'invalid response shortcode',
          },
          raw: requestRes,
        })
