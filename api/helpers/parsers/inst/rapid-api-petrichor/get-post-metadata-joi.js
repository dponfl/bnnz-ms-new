...
const moduleName = 'parsers:inst:rapid-api-petrichor:get-post-metadata-joi';


module.exports = {


  friendlyName: 'parsers:inst:rapid-api-petrichor:get-post-metadata-joi',


  description: 'parsers:inst:rapid-api-petrichor:get-post-metadata-joi',
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      const parserUrl = sails.config.custom.config.parsers.inst.rapidApiPetrichor.url;
      const parserAction = sails.config.custom.config.parsers.inst.rapidApiPetrichor.paths.getPostMetadata;

      const momentStart = moment();

      const options = {
        uri: `${parserUrl}${parserAction}/${shortCode}`,
        method: 'GET',
        headers: {
          "x-rapidapi-host": sails.config.custom.config.parsers.inst.rapidApiPetrichor.apiHost,
          "x-rapidapi-key": sails.config.custom.config.parsers.inst.rapidApiPetrichor.apiKey,
        },
        json: true,
      };
      ...
      const media = _.get(requestRes, 'graphql.shortcode_media', null);

      if (
        _.isNil(media)
        && _.isNil(requestRes.detail)
      ) {
      ...
      const mediaId = _.get(requestRes, 'graphql.shortcode_media.id', null);
      ...
      if (mediaId == null) {
        ...
        await LogProcessor.error({
          message: sails.config.custom.INST_PARSER_WRONG_RESPONSE_DATA.message + ': No "graphql.shortcode_media.id"',
          clientGuid,
          accountGuid,
          // requestId: null,
          // childRequestId: null,
        ...
        return exits.success({
          ...
          message: `${moduleName} performed with error`,
          payload: {
            error: 'No graphql.shortcode_media.id',
          },
          raw: requestRes,
        })
